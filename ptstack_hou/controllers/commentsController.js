...
export const getComments = async (req, res) => {
  ...
  try {
    ...
    const { page = 1, pageSize = 20, sortBy = 'created_at', order = 'desc' } = req.query
...
export const createComment = async (req, res) => {
  ...
  try {
    ...
    const currentUser = await execute(
      'SELECT id, public_id, nickname, username FROM users WHERE id = ?',
      [userId],
    )
    ...
    if (replyToUserId) {
      const [replyToUser] = await execute('SELECT id FROM users WHERE public_id = ?', [
        replyToUserId,
      ])
      if (replyToUser) {
    ...
    const result = await execute(
      ...
      [
        ...
        trimmedContent,
        parentId || null,
        internalReplyToUserId || null,
        replyToCommentId || null,
      ],
    )

    await execute('UPDATE articles SET comment_count = comment_count + 1 WHERE id = ?', [
      internalArticleId,
    ])
    ...
    if (
      ...
      internalReplyToUserId !== userId &&
      internalReplyToUserId !== authorId
    ) {
      await execute(
        `INSERT INTO notifications (user_id, type, content, related_id, is_read) 
         VALUES (?, ?, ?, ?, 0)`,
