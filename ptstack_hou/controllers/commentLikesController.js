...
export const toggleCommentLike = async (req, res) => {
  try {
    ...
    if (existing.length === 0) {
      await execute('INSERT INTO comment_likes (comment_id, user_id) VALUES (?, ?)', [
        commentId,
        userId,
      ])
      ...
      await execute('DELETE FROM comment_likes WHERE comment_id = ? AND user_id = ?', [
        commentId,
        userId,
      ])
      await execute('UPDATE comments SET like_count = GREATEST(like_count - 1, 0) WHERE id = ?', [
        commentId,
      ])
      const result = await execute('SELECT like_count FROM comments WHERE id = ?', [commentId])
