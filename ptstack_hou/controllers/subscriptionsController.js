...
export const toggleSubscription = async (req, res) => {
  try {
    ...
    if (existing) {
      await execute('DELETE FROM subscriptions WHERE follower_id = ? AND following_id = ?', [
        followerId,
        internalFollowingId,
      ])

      await execute('UPDATE users SET follower_count = follower_count - 1 WHERE id = ?', [
        internalFollowingId,
      ])
      await execute('UPDATE users SET following_count = following_count - 1 WHERE id = ?', [
        followerId,
      ])

      res.json({ message: '已取消订阅', isSubscribed: false })
    } else {
      await execute('INSERT INTO subscriptions (follower_id, following_id) VALUES (?, ?)', [
        followerId,
        internalFollowingId,
      ])
...
export const getUserFollowers = async (req, res) => {
  try {
    ...
    if (search) {
      ...
      countWhereClause +=
        ' AND EXISTS (SELECT 1 FROM users u2 WHERE u2.id = s.follower_id AND u2.username LIKE ?)'
    ...
    const usersWithSubscriptionStatus = await Promise.all(
      followers.map(async (follower) => {
        ...
        if (currentUserId) {
          const [followerUser] = await execute('SELECT id FROM users WHERE public_id = ?', [
            follower.id,
          ])
          if (followerUser) {
...
export const getUserFollowing = async (req, res) => {
  try {
    ...
    if (search) {
      ...
      queryParams.push(searchPattern)
      countWhereClause +=
        ' AND EXISTS (SELECT 1 FROM users u2 WHERE u2.id = s.following_id AND u2.username LIKE ?)'
      countParams.push(searchPattern)
    }
    ...
    const usersWithSubscriptionStatus = await Promise.all(
      following.map(async (followingUser) => {
        let isSubscribed = false
        if (currentUserId) {
          const [followingUserInternal] = await execute(
