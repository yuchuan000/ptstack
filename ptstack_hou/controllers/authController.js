...
export const sendEmailVerification = async (req, res, next) => {
  try {
    ...
    // 检查该邮箱是否已被注册
    const existingUsers = await execute('SELECT id FROM users WHERE email = ?', [email])
    ...
    if (recentRecords.length > 0) {
      ...
      // 超过冷却时间，删除旧记录
      await execute('DELETE FROM email_verifications WHERE id = ?', [recentRecords[0].id])
    }
...
export const verifyEmailCode = async (req, res) => {
  try {
    ...
    if (new Date(verification.verification_code_expires_at) < new Date()) {
      // 删除过期记录
      await execute('DELETE FROM email_verifications WHERE id = ?', [verification.id])
...
export const register = async (req, res, next) => {
  try {
    ...
    // 检查用户名是否已存在
    const existingUsers = await execute('SELECT id FROM users WHERE username = ? OR email = ?', [
      username,
      email,
    ])
