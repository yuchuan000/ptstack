...
async function addMissingFields() {
  ...
  try {
    ...
    const requiredFields = [
      {
        name: 'overview',
        definition: 'VARCHAR(500) DEFAULT NULL COMMENT "公告概述" AFTER content',
      },
      { name: 'priority', definition: 'INT DEFAULT 0 COMMENT "优先级" AFTER overview' },
      {
        name: 'is_marquee',
        definition: 'TINYINT DEFAULT 0 COMMENT "是否首页顶部通告" AFTER priority',
      },
      {
        name: 'target_type',
        definition: 'VARCHAR(20) DEFAULT "all" COMMENT "目标类型" AFTER is_marquee',
      },
      ...
      {
        name: 'created_by',
        definition: 'INT DEFAULT NULL COMMENT "创建者ID" AFTER delivery_methods',
      },
      ...
      if (!existingColumns.includes(field.name)) {
        try {
          await connection.query(
            `ALTER TABLE announcements ADD COLUMN ${field.name} ${field.definition}`,
          )
